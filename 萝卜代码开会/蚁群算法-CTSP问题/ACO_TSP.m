...
iter=1;%初始迭代次数
iter_max=200;%最大迭代次数
Route_best=zeros(iter_max,n);%各代最佳矩阵
...
while iter<=iter_max
    ...
    for i=1:m
        %逐个城市路径选择
        for j=2:n
            tabu=Table(i,1:(j-1));%已访问过的城市集合(禁忌表)
            allow_index=~ismember(citys_index,tabu);%citys_index - tabu
...
xlabel('城市位置横坐标');
ylabel('城市位置纵坐标');
title(['蚁群算法优化路径最短距离:',num2str(Shortest_Length)])
